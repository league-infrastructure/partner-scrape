...
Ticket 011-003 adds a `"tba"` entry once `sources/tba.py` exists, plus
the per-source failure isolation Migration Concerns calls for (a
missing/401 `TBA_KEY` must degrade to FTC-only output, never fail the
whole run). This ticket's own per-source try/except below already
gives every source that isolation -- it is not later-ticket-only
behavior, just currently exercised by a single source.
"""
...
#: `ftc-sd.toml`'s `adapter_type = "ftcscout"`) -> the `TeamSource`
#: instance that handles it. See this module's own docstring for why
#: this is a private lookup, not an `ADAPTERS`-shaped public registry.
    ...
    *,
    ...
    Args:
        ...
        source: when given, restricts the run to the single acquisition
            source whose `adapter_type` matches (e.g. `"ftcscout"`) --
            the CLI's `--source` flag. Unlike `pipeline.run()`'s
            `source_id` filter (which matches a registry file's stem),
            this matches the *kind* of source, since a future TBA
            outage (ticket 011-003) needs to be isolated by acquisition
