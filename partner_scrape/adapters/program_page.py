"""The ``program_page`` Adapter: one individually-registered program page.

Sprint 027 ticket 003. See ``adapters/DESIGN.md``'s sprint 027 section and
sprint.md's SUC-031: given one registered program page URL (a paid summer
research placement, an internship, a scholarship, or similar
application-window program), fetch it and call the ticket-002 LLM
extraction client (``program_llm.py``) to turn its raw prose into a
canonical ``Event`` carrying deadline-first fields -- ``kind``, ``start``/
``end``, ``eligibility``, ``opportunity_type`` -- that no structured API
publishes and no deterministic ``extract/`` ladder rung could recover.
...
§3): unlike every other adapter, ``ProgramPageAdapter`` accepts optional
...
``eligibility``, ``is_open``, and ``opportunity_type``. This adapter does
not invent one; ``Event.description`` is simply left unset for a
``program_page`` record.
...
adapter's job is only to map the LLM's structured output onto an ``Event``.
"""
...
class ProgramPageAdapter:
    ...
    def extract(self, raw: RawResponse, source: SourceConfig) -> Iterable[Event]:
        ...
        extraction cache by URL + content hash; on a miss, call the
        injected ``ProgramLLMClient`` and store the result. The
        source's ``config.program_kind`` (required; ``"internship"`` or
        ``"program"``) sets ``Event.kind`` -- a missing or invalid value
        is logged and skipped, never raised, matching this adapter's
        general per-source error-isolation stance.
        """
        if raw.status != 200:
            logger.warning(
        ...
        result = self.cache.lookup(raw.ref.url, raw.body)
        if result is None:
            result = self.llm_client.extract_program(raw.ref.url, raw.body)
            self.cache.store(raw.ref.url, raw.body, result)
