...
class TestAcquisitionPolicyThreading:
    ...
    def test_sources_acquisition_policy_reaches_both_probe_and_page_fetch(self):
        fetcher = _two_page_fetcher()
        source = _source(acquisition_policy={"rate_limit_seconds": 0.75, "respect_robots": False})

        run(source, fetcher)

        assert fetcher.policy_calls[PROBE_URL] == (0.75, False)
        assert fetcher.policy_calls[PAGE2_URL] == (0.75, False)
