...
class TestRunDirectoryRealFixtureData:
    ...
    def test_dry_run_reports_thirty_clubs_with_no_network(self, tmp_path):
        ...
        # already applies to Places. Sprint 032 registered six more
        # real club_static_roster entries (CyberPatriot 3, Civil Air
        # Patrol 7, Sea Cadets 4, 4-H 14, Science Olympiad 24, Girls
    ...
    def test_every_real_school_hosted_club_resolves_to_school_precision_never_a_guess(
        ...
        # school-hosted (hack-club); civil-air-patrol's own honest
        # zip/city fallthrough is covered by
        # TestRealCivilAirPatrolGeocoding below instead. CyberPatriot
        # migrated to teams.model.Team (sprint 036 ticket 002) and no
        # longer appears in this registry at all.
        _write_real_partners_fixture(tmp_path / "unused")
        payload = run_directory(
            fetcher=_NeverCalledFetcher(), dry_run=True, site_dir=tmp_path / "unused"
        )

        school_hosted_types = {"hack-club"}
    ...
    def test_helix_charter_is_the_one_real_chapter_flagged_needs_review(self, tmp_path):
        # Real-world residue one curated host_school value genuinely
        # hits: Hack Club's "Helix Charter High School" (CDE's own
        # record is named "Helix High" -- a legitimate rung-3
        # same-city fuzzy match, not an exact one), flagged rather than
        # silently trusted. Science Olympiad's own second such flag
        # (San Dieguito High School Academy) migrated to
        # `teams.model.Team` along with the rest of that roster (sprint
        # 036 ticket 002) -- see `tests/teams/
        # test_migration_science_olympiad_cyberpatriot.py` for the
        # proof that flag survived the move unchanged. See
        # directory/DESIGN.md.
        _write_real_partners_fixture(tmp_path / "unused")
        payload = run_directory(
            fetcher=_NeverCalledFetcher(), dry_run=True, site_dir=tmp_path / "unused"
...
class TestRealGirlsWhoCodeGeocoding:
    ...
    resolved at school precision for both the CyberPatriot and Science
    Olympiad rosters before their sprint 036 ticket 002 migration to
    `teams.model.Team`, so this one entry is expected to resolve the
    same way, not a guess."""
