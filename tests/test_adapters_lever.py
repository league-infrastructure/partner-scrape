...
class TestAcquisitionPolicyThreading:
    def test_sources_acquisition_policy_reaches_fetcher_get(self):
        fetcher = _fetcher()
        source = _source(acquisition_policy={"rate_limit_seconds": 1.5, "respect_robots": False})

        run(source, fetcher)

        assert fetcher.policy_calls[POSTINGS_URL] == (1.5, False)
    ...
    def test_source_with_no_acquisition_policy_still_gets_polite_fetcher_defaults(self):
        ...
        assert fetcher.policy_calls[POSTINGS_URL] == (DEFAULT_RATE_LIMIT_SECONDS, True)
